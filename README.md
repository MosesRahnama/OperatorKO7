...
- `OperatorKO7/Meta/DependencyPairs_FirstOrderEngine.lean`:
  engine-level frontend for the same extractor; packages an internal first-order TRS / DP engine as one object carrying `Rule`, `rules`, `lhs`, and `rhs`, then reexports the same extracted-call-graph and SCC-search / contradiction surface directly from that engine object. The same file now also contains the typeclass-level engine view (`HasFiniteFirstOrderView`) and the raw-term bridge (`HasFirstOrderTermView`, `HasFiniteRawFirstOrderView`) for internal engines that keep their own term syntax.
...
- `OperatorKO7/Meta/DependencyPairs_FiniteCarrierView.lean`:
  consolidated finite-carrier bridge for internal engines whose rules already form a finite type; it removes the explicit rule-array layer by enumerating the carrier, then packages the canonical first-order surface, the finite raw rule-carrier bridge (`HasFiniteCarrierRawFirstOrderView`), the smaller head-view surface (`FiniteCarrierHeadEngine`, `HasFiniteCarrierHeadView`), and the extracted-data carrier bridge (`FiniteCarrierExtractedEngine`, `HasFiniteCarrierExtractedView`) in one module.
- `OperatorKO7/Meta/DependencyPairs_KernelFirstOrder.lean`:
  internal KO7-symbol frontend for the generic first-order extraction layer; packages the eight full KO7 root rules over an internal symbol type, exposes them through `HasFiniteRawFirstOrderView`, derives the smaller `HasFiniteHeadRuleView` via the generic head-view adapter, and then feeds the same system through the consolidated finite-carrier stack: finite rule carrier (`RuleId`), finite raw rule-carrier, finite head-carrier, and finite extracted-data carrier.
